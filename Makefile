# This Makefile is for the LogFilter extension to perl.
#
...
#     NAME => q[LogFilter]
#     PREREQ_PM => { File::Tail=>q[0], IO::File=>q[0], perl=>q[5.028] }
#     TEST_REQUIRES => {  }
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/LogFilter.pm $(INST_MAN3DIR)/LogFilter.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''A simple log filter'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: LogFilter' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "A simple log filter",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "LogFilter",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="LogFilter" VERSION="0.03">' > LogFilter.ppd
	...
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-thread-multi-2level-5.36" />' >> LogFilter.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> LogFilter.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> LogFilter.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> LogFilter.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/LogFilter.pm' 'blib/lib/LogFilter.pm' 
